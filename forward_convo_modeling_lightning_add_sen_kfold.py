...
class AbuseDataset(Dataset):
  ...
  def __init__(self, reviews, targets, c_list, c_num, f_list, f_num, tokenizer, max_len, context_window, ids):
  ...
  def __getitem__(self, item):
    ...
    f_num = self.f_num[item]
    f_list = self.f_list[item]
    idx = self.ids[item]
    ...
    c=["<pad>" for i in range(self.context_window)]
    n = f_num if f_num<self.context_window else self.context_window
    f_context_input_ids = []
    f_context_attention_mask = []
    for i in range(n):
      c[i] = f_list[i]
    for i in range(0,self.context_window):
      ...
      return_attention_mask=True,
      return_tensors='pt')
      f_context_input_ids.append(encoding_context['input_ids'].flatten())
      f_context_attention_mask.append(encoding_context['attention_mask'].flatten())
    ...
    return {
      ...
      'f_context_input_ids': torch.stack(context_input_ids),
      'f_context_attention_masks': torch.stack(context_attention_mask),
      'context_num': c_num,
      'f_context_num': f_num,
      'ids': idx
    }
...
class Abuse_lightning(LightningModule):
  ...
  def __init__(self, df_train,df_val,df_test, config):
    ...
    self.f_attention = GeneralAttention(self.bert.config.hidden_size, self.config['context_window'])
    self.f_attention = self.f_attention.to(self.device)
  ...
  def __retrieve_dataset(self, train=True, val=True, test=True):
    ...
    if train:
      ...
      f_num = self.df_train.f_context_num.astype(int)
      ...
      f_context_list = []
      for i, val in enumerate(f_num):
        context = []
        for j in range(val):
          context.append(self.df_train["f_context"+str(j+1)][i])
        f_context_list.append(context)
      
      ds = AbuseDataset(reviews=self.df_train.comment.to_numpy(), targets=self.df_train.Score.to_numpy(), c_list = context_list,
       c_num = self.df_train.context_num.to_numpy(), f_list = f_context_list,
       f_num = self.df_train.f_context_num.to_numpy(), tokenizer=self.tokenizer,
       max_len=self.max_len, context_window = self.config['context_window'], ids = self.df_train.idx)
    # if val:
    ...
    if test:
      ...
      f_num = self.df_test.f_context_num.astype(int)
      ...
      for i, val in enumerate(f_num):
        context = []
        for j in range(val):
          context.append(self.df_test["f_context"+str(j+1)][i])
        f_context_list.append(context)

      ds = AbuseDataset(reviews=self.df_test.comment.to_numpy(), targets=self.df_test.Score.to_numpy(), c_list = context_list,
       c_num = self.df_test.context_num.to_numpy(),f_list = f_context_list,
       f_num = self.df_test.f_context_num.to_numpy(), tokenizer=self.tokenizer,max_len=self.max_len, 
       context_window = self.config['context_window'], ids = self.df_test.idx)
    return ds
  ...
  def training_step(self, d, batch_idx):
    ...
    f_context_input_ids = d["f_context_input_ids"].to(self.device)
    f_context_attention_masks = d["f_context_attention_masks"].to(self.device)
    f_context_num = d['f_context_num'].to(self.device)
    ...
    #FORWARD CONTEXT
    out_encoding = []
    for i in range(len(f_context_input_ids)):
      c = self.forward(input_ids=f_context_input_ids[i].to(self.device),attention_mask=f_context_attention_masks[i].to(self.device))
      out_encoding.append(c)
    mask = torch.zeros([input_ids.shape[0],self.config['context_window']])
    for i in range(len(f_context_num)):
      arr = np.zeros(self.config['context_window'])
      arr[:f_context_num[i]] = 1
      mask[i] = torch.tensor(arr)
    mask = mask.to(self.device)
    f_weighted = self.f_attention.forward(out_encoding, mask, self.batch_size, self.device)

    main_context = outputs.add(weighted.squeeze(dim=1))
    main_context = main_context.add(f_weighted.squeeze(dim=1))
...
if __name__ == "__main__":
  ...
  trainer.fit(model)
  # trainer = pl.Trainer(gpus = 1, progress_bar_refresh_rate=0, max_epochs= config['num_epochs'], checkpoint_callback=checkpoint_callback, accumulate_grad_batches=4)
