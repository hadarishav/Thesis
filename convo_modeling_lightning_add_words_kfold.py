...
class Abuse_lightning(LightningModule):
  ...
  def __retrieve_dataset(self, train=True, val=True, test=True):
    ...
    #     c_num = self.df_val.context_num.to_numpy(), tokenizer=self.tokenizer,max_len=self.max_len, context_window = self.config['context_window'], ids = self.df_val.idx)
    if test:
  ...
  def training_step(self, d, batch_idx):
    ...
    mask = torch.zeros([input_ids.shape[0], self.config['max_len'] * self.config['context_window']])
    for i in range(len(context_num)):
      a = context_attention_masks[i].reshape(-1)
      mask[i] = a
